...
describe('HfModelCard', () => {
  it('shows the model name when the truncated name is hovered', async () => {
    const user = userEvent.setup();
    renderCard();

    await user.hover(screen.getByRole('heading', { name: model.name }));

    expect(await screen.findByRole('tooltip')).toHaveTextContent(model.name);
  });

  it('makes the model name available from the keyboard', async () => {
    const user = userEvent.setup();
    renderCard();
